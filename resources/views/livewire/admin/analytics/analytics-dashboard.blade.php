...
<div wire:poll.15s>
    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-2">
            ...
            <select wire:model.live="period" class="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-white/10 rounded-xl px-4 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-indigo-500 focus:outline-none">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="glass-card rounded-2xl p-6 hover:-translate-y-1 transition-transform duration-300">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-xs font-bold {{ $viewsGrowth >= 0 ? 'text-emerald-400 bg-emerald-400/10' : 'text-red-400 bg-red-400/10' }} px-2 py-1 rounded-lg">
        ...
        <div class="glass-card rounded-2xl p-6 hover:-translate-y-1 transition-transform duration-300">
            ...
            <div class="text-3xl font-black text-zinc-900 dark:text-white mb-1">{{ number_format($uniqueVisitors) }}</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 glass-card rounded-2xl p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-4 p-3 rounded-xl {{ $index < 3 ? 'bg-indigo-500/5 border border-indigo-500/10' : 'hover:bg-zinc-100 dark:hover:bg-zinc-800/30' }} transition-colors">
                    <div class="w-8 h-8 rounded-lg {{ $index === 0 ? 'bg-yellow-500/20 text-yellow-400' : ($index === 1 ? 'bg-zinc-300/20 text-zinc-400' : ($index === 2 ? 'bg-orange-500/20 text-orange-400' : 'bg-zinc-800/30 text-zinc-500')) }} flex items-center justify-center text-sm font-black shrink-0">
                        {{ $index + 1 }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <h4 class="text-sm font-semibold text-zinc-900 dark:text-white truncate">{{ $article->title }}</h4>
                        <p class="text-xs text-zinc-500">by {{ $article->author?->name ?? 'Unknown' }} · {{ $article->category?->name ?? 'Uncategorized' }}</p>
                    </div>
                    <div class="text-right shrink-0">
                        <div class="text-sm font-bold text-zinc-900 dark:text-white">{{ number_format($article->period_views) }}</div>
        ...
        <div class="space-y-8">
            ...
            <div class="glass-card rounded-2xl p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-indigo-500/20 shrink-0">
                            {{ substr($author->name, 0, 1) }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-zinc-900 dark:text-white truncate">{{ $author->name }}</p>
                            <p class="text-xs text-zinc-500">{{ $author->published_count }} articles</p>
                        </div>
                        <div class="text-sm font-bold text-zinc-900 dark:text-white">{{ number_format($author->view_count) }} <span class="text-xs text-zinc-500 font-normal">views</span></div>
                    </div>
                    @empty
    ...
    <div class="mt-8 glass-card rounded-2xl p-6">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-4 p-3 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800/30 transition-colors">
                <div class="w-8 h-8 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-xs text-zinc-700 dark:text-white font-bold shrink-0">
                    {{ substr($view->user?->name ?? 'G', 0, 1) }}
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-sm text-zinc-900 dark:text-zinc-300">
                        <span class="font-semibold text-zinc-900 dark:text-white">{{ $view->user?->name ?? 'Guest' }}</span>
                        viewed
                        <span class="font-semibold text-indigo-500 dark:text-indigo-400">{{ Str::limit($view->article?->title ?? 'Unknown', 40) }}</span>
                    </p>
                </div>

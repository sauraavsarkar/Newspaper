...
<aside class="glass-sidebar w-64 h-full flex flex-col shrink-0">
    ...
    <nav class="flex-1 py-6 px-3 flex flex-col gap-1 overflow-y-auto">
        ...
        @if(auth()->user()->can('manage users') || auth()->user()->can('assign roles'))
            ...
            <a href="{{ route('admin.activity') }}" wire:navigate class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all {{ request()->routeIs('admin.activity') ? 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20' : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800/50' }}">
                <svg class="w-5 h-5 {{ request()->routeIs('admin.activity') ? 'text-indigo-600 dark:text-indigo-400' : 'text-zinc-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                Activity Feed
            </a>
        @endif
    </nav>
